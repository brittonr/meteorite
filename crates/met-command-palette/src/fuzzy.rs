...
//!
//! Ported from subwayrat's `rat-fuzzy` — pure logic, no UI deps.
...
pub struct ScoredMatch {
    ...
    pub score: i32,
    /// Character indices in the candidate text that matched the query.
...
pub fn fuzzy_score(text: &str, query: &str) -> Option<ScoredMatch> {
    ...
    // Greedy forward pass — check feasibility and record positions.
...
mod tests {
    ...
    fn consecutive_beats_scattered() {
        let c = fuzzy_score("abcdef", "abc").unwrap();
        let s = fuzzy_score("a_b_c_def", "abc").unwrap();
